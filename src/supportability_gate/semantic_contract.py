...
def request_payload(packet: EvidencePacket) -> dict[str, Any]:
    ...
    instructions = (
        ...
        "boundaries fail closed and target code cannot execute in the local evaluator or semantic "
        "verifier. Owner-authorized characterization drivers may execute only through fixed Python "
        "or Node vectors in separate GitHub-hosted base/head jobs; that isolated execution is "
        "required, while local, self-hosted, configurable, or arbitrary execution blocks. Runtime "
        "and protected-merge proof "
