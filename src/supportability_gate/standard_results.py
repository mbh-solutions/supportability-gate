"""Compose and enforce one independently visible result per Supportability Standard."""
...
def _compose(arguments: argparse.Namespace) -> int:
...
def _enforce(arguments: argparse.Namespace) -> int:
    try:
        payload = validate_payload(
            _read_json(Path(arguments.input), "MISSING_STANDARD_RESULTS"), _identity(arguments)
        )
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="supportability-standard-results")
    commands = parser.add_subparsers(dest="command", required=True)
    compose = commands.add_parser("compose")
    _identity_arguments(compose)
    compose.add_argument("--pull-number", required=True, type=int)
    compose.add_argument("--complexity-result", required=True)
    compose.add_argument("--characterization-result", required=True)
    compose.add_argument("--refactor-result", required=True)
    compose.add_argument("--quality-provenance", required=True)
    compose.add_argument("--output", required=True)
    enforce = commands.add_parser("enforce")
    _identity_arguments(enforce)
    enforce.add_argument("--input", required=True)
    enforce.add_argument("--standard", required=True, type=int, choices=range(1, 9))
    return parser


def main(argv: list[str] | None = None) -> int:
    """Compose or enforce standard results at an isolated workflow boundary."""
    arguments = _parser().parse_args(argv)
    return _compose(arguments) if arguments.command == "compose" else _enforce(arguments)
